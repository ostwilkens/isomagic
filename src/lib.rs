...
impl Renderer {
    ...
    pub fn render_all(&mut self, options: Options) -> Result<()> {
        let models = options
            .model
            .map(|model| vec![model])
            .unwrap_or_else(|| (0..self.vox.models.len()).collect());
        let sides = options
            .side
            .map(|side| vec![side])
            .unwrap_or_else(Side::all);
        let views = options
            .view
            .map(|view| vec![view])
            .unwrap_or_else(View::all);
    ...
    fn render(
        ...
        let image = match *view {
            ...
            View::TwentyTwoPointFiveIso => {
                ModelRenderer::new(self, model, 3, 3).render_22_5_iso(side)
            }
        };

        if !output.exists() {
            create_dir_all(&output)
                .chain_err(|| format!("Failed to create directory '{}'.", output.display()))?;
        }
